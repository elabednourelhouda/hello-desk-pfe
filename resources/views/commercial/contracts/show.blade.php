...
@section('title', 'Détail contrat - Espace commercial')
...
@php
    ...
    $hasCommercialPayments = \Illuminate\Support\Facades\Route::has('commercial.payments.index');
    $hasCommercialPaymentCreate = \Illuminate\Support\Facades\Route::has('commercial.payments.create');
    $hasCommercialPaymentShow = \Illuminate\Support\Facades\Route::has('commercial.payments.show');
    $hasCommercialMarkAsPaid = \Illuminate\Support\Facades\Route::has('commercial.payments.markAsPaid');
@endphp
...
<div class="mx-auto max-w-6xl px-6 py-8">
        ...
        <div class="mt-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Contrat lié à une réservation de votre périmètre commercial.
                </p>
            </div>
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                @if($contract->pdf_path)
                    ...
                    <a href="{{ route('commercial.contracts.document', $contract) }}"
        ...
        <aside class="rounded-3xl border border-purple-100 bg-gradient-to-br from-white via-purple-50/40 to-blue-50/40 p-6 shadow-sm">
            ...
            <div class="mt-5 space-y-4 text-sm">
                ...
                <div>
                    ...
                    <p class="mt-1 text-gray-700">
                        {{ $reservationStatusLabels[$contract->reservation?->status] ?? $contract->reservation?->status ?? 'Non précisé' }}
                    </p>
                </div>

                @if($contract->reservation)
                    <a href="{{ route('commercial.reservations.show', $contract->reservation) }}"
    ...
    <section class="mt-6 rounded-3xl border border-amber-100 bg-gradient-to-br from-white via-amber-50/40 to-rose-50/30 p-6 shadow-sm">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            @else
                <span class="inline-flex justify-center rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-500">
                    Module paiements à relier
                </span>
            @endif
        </div>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        @else
                                            <span class="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-500">
                                                Voir
                                            </span>
                                        @endif

                                        @if($payment->status !== 'paid')
                                            @if($hasCommercialMarkAsPaid)
