...
@section('title', 'Carte interactive - Espace commercial')
...
<div class="min-h-screen bg-[#f6f8f6]">
    <div class="mx-auto max-w-7xl px-6 py-8">
            ...
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-3xl text-sm leading-6 text-gray-500">
                        Visualisez les espaces de votre périmètre commercial par campus et par étage,
                        consultez leur disponibilité et accédez rapidement à la réservation.
                    </p>
                </div>
        ...
        @if(isset($hasCommercialScope) && ! $hasCommercialScope)
        <div class="mt-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">
            Vous n’avez aucune affectation active. Vous pouvez consulter la carte en lecture seule, mais vous ne pouvez pas créer de réservation.
        </div>
        @endif
        ...
        <section class="mt-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <section class="mt-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <form method="GET"
                    action="{{ route('commercial.interactive-map.index') }}#map-zone"
                    ...
                    <div>
                        ...
                        <select name="campus_id"
                            ...
                            @empty
                            <option value="">Aucun campus affecté</option>
                            @endforelse
                        </select>
                    </div>
            ...
            <section class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                <div class="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                    <p class="text-xs font-bold uppercase text-emerald-700">Disponible</p>
                    <p class="mt-2 text-2xl font-bold text-emerald-800">{{ $statusCounts['available'] }}</p>
                </div>

                <div class="rounded-2xl border border-sky-200 bg-sky-50 p-4">
                    <p class="text-xs font-bold uppercase text-sky-700">Réservé</p>
                    <p class="mt-2 text-2xl font-bold text-sky-800">{{ $statusCounts['reserved'] }}</p>
                </div>

                <div class="rounded-2xl border border-slate-200 bg-slate-100 p-4">
                    <p class="text-xs font-bold uppercase text-slate-700">Occupé</p>
                    <p class="mt-2 text-2xl font-bold text-slate-800">{{ $statusCounts['occupied'] }}</p>
                </div>

                <div class="rounded-2xl border border-rose-200 bg-rose-50 p-4">
                    <p class="text-xs font-bold uppercase text-rose-700">Indisponible</p>
                    <p class="mt-2 text-2xl font-bold text-rose-800">{{ $statusCounts['unavailable'] }}</p>
                </div>

                <div class="rounded-2xl border border-orange-200 bg-orange-50 p-4">
                    <p class="text-xs font-bold uppercase text-orange-700">Maintenance</p>
                    <p class="mt-2 text-2xl font-bold text-orange-800">{{ $statusCounts['maintenance'] }}</p>
                </div>
            </section>
                    ...
                    <div class="mb-5 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-xs font-semibold text-[#284625]">
                                {{ $currentCampus?->name ?? 'Campus non précisé' }}
                                —
                                {{ $currentFloor?->name ?? 'Étage non précisé' }}
                            </p>
                            @endif
                        </div>
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-inner">
                        ...
                        <div class="flex min-h-[460px] items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-white">
                            <div class="text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-500">
                                    Aucun espace actif n’est disponible pour ce campus et cet étage dans votre périmètre.
                                </p>
                            </div>
                        </div>
                        @endif
...
<script>
    function selectSpace(button) {
        ...
        if (button.dataset.reserveUrl) {
            ...
            if (button.dataset.canManage !== '1') {
                cannotReserveMessage.textContent = 'Cet espace est hors de votre périmètre. Consultation en lecture seule uniquement.';
            } else {
                cannotReserveMessage.textContent = 'Cet espace ne peut pas être réservé actuellement.';
            }
