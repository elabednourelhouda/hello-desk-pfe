...
@section('title', 'Modifier ' . $space->name . ' - Administration')
...
<div class="min-h-screen bg-gray-50">
    <div class="mx-auto max-w-3xl px-6 py-8">
        ...
        <p class="text-sm font-medium text-[#284625]">
            ...
            /
            <a href="{{ route('admin.spaces.show', $space) }}" class="hover:underline">{{ $space->name }}</a>
            /
            Modifier
        </p>
        <h1 class="mt-1 text-3xl font-bold text-gray-900">Modifier « {{ $space->name }} »</h1>
        ...
        <form method="POST"
              action="{{ route('admin.spaces.update', $space) }}"
                ...
                <div>
                    ...
                    <select name="campus_id"
                        ...
                        @foreach($campuses as $campus)
                            <option value="{{ $campus->id }}" @selected(old('campus_id', $space->campus_id) == $campus->id)>
                                {{ $campus->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="floor_id"
                        ...
                        @foreach($floors as $floor)
                            <option value="{{ $floor->id }}" @selected(old('floor_id', $space->floor_id) == $floor->id)>
                ...
                <div>
                    ...
                    <select name="space_type_id"
                        ...
                        @foreach($spaceTypes as $type)
                            <option value="{{ $type->id }}" @selected(old('space_type_id', $space->space_type_id) == $type->id)>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        @foreach($statuses as $value => $label)
                            <option value="{{ $value }}" @selected(old('status', $space->status) === $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mt-4 grid gap-5 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="number"
                           ...
                           min="0"
                           name="surface"
                           value="{{ old('surface', $space->surface) }}"
                           class="h-12 w-full rounded-xl border border-gray-300 px-4 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">
                    @error('surface')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-4 grid gap-5 md:grid-cols-4">
                <div>
                    ...
                    <input type="number"
                           ...
                           id="price_per_hour"
                           name="price_per_hour"
                           value="{{ old('price_per_hour', $space->price_per_hour) }}"
                           class="h-12 w-full rounded-xl border border-gray-300 px-4 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">
                ...
                <div>
                    ...
                    <input type="number"
                           ...
                           value="{{ old('price_per_half_day', $space->price_per_half_day) }}"
                           placeholder="Auto : 50% du prix / jour"
                           class="h-12 w-full rounded-xl border border-gray-300 px-4 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">
                    <p class="mt-1 text-xs text-gray-500">
                        Laissez vide pour réappliquer automatiquement 50% du prix / jour.
                    </p>
                ...
                <div>
                    ...
                    <input type="number"
                           ...
                           id="price_per_day"
                           name="price_per_day"
                           value="{{ old('price_per_day', $space->price_per_day) }}"
                           class="h-12 w-full rounded-xl border border-gray-300 px-4 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">
                    @error('price_per_day')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number"
                           ...
                           value="{{ old('price_per_month', $space->price_per_month) }}"
                           class="h-12 w-full rounded-xl border border-gray-300 px-4 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">
                ...
                @forelse($accessories as $accessory)
                    <label class="flex items-center gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
                        <input type="checkbox"
                               name="accessory_ids[]"
                               value="{{ $accessory->id }}"
                               @checked(collect(old('accessory_ids', $selectedAccessoryIds))->contains($accessory->id))
            ...
            <div class="mt-4 grid gap-5">
                <div>
                    ...
                    <textarea name="description"
                              ...
                              class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">{{ old('description', $space->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes"
                              ...
                              class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">{{ old('notes', $space->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <label class="flex items-center gap-3">
                    <input type="checkbox"
                           name="is_active"
                           value="1"
                           {{ old('is_active', $space->is_active) ? 'checked' : '' }}
                           class="h-5 w-5 rounded border-gray-300 text-[#284625] focus:ring-[#284625]">
