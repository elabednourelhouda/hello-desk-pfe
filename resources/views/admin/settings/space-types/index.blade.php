...
@section('title', 'Types d’espaces - Administration')
    ...
    <div class="mx-auto max-w-5xl px-6 py-8">
        ...
        <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-3xl font-bold text-gray-900">Types d’espaces</h1>
                <p class="mt-2 text-sm text-gray-500">
                    Ces valeurs alimentent le champ « Type d’espace » partout dans l’application
                    (fiche espace, formulaire prospect, filtres).
                </p>
            </div>

            <a href="{{ route('admin.settings.space-types.create') }}"
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold
                                        ...
                                        {{ $spaceType->is_active ? 'Actif' : 'Inactif' }}
                                    </span>
                                </td>

                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.settings.space-types.edit', $spaceType) }}"
                                        ...
                                        <form method="POST"
                                              action="{{ route('admin.settings.space-types.toggleActive', $spaceType) }}">
                                            @csrf
                                            @method('PATCH')
                                            ...
                                            <button type="submit"
                                                ...
                                                {{ $spaceType->is_active ? 'Désactiver' : 'Activer' }}
                                            </button>
                                        </form>

                                        <form method="POST"
                                              action="{{ route('admin.settings.space-types.destroy', $spaceType) }}"
                                              onsubmit="return confirm('Supprimer définitivement ce type d’espace ?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-14 text-center text-sm text-gray-500">
                                    Aucun type d’espace enregistré pour le moment.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
