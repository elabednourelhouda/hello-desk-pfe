...
@section('title', 'Ajouter un type de durée - Administration')
...
<div class="min-h-screen bg-gray-50">
    <div class="mx-auto max-w-2xl px-6 py-8">
        ...
        <h1 class="mt-1 text-3xl font-bold text-gray-900">Ajouter un type de durée</h1>

        <form method="POST"
              action="{{ route('admin.settings.reservation-duration-types.store') }}"
            ...
            <div class="grid gap-5">
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           value="{{ old('code') }}"
                           required
                           placeholder="Ex: quarterly"
                           class="h-12 w-full rounded-xl border border-gray-300 px-4 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">
                    <p class="mt-1 text-xs text-gray-500">
                        Valeur technique stockée en base (lettres, chiffres, tirets, underscores). Ne peut plus être
                        changée aussi facilement une fois des réservations créées avec ce code.
                    </p>
                        ...
                        @forelse($spaceTypes as $spaceType)
                            <label class="flex items-center gap-2">
                                <input type="checkbox"
                                       name="space_type_ids[]"
                                       value="{{ $spaceType->id }}"
                                       {{ in_array($spaceType->id, old('space_type_ids', [])) ? 'checked' : '' }}
                ...
                <label class="flex items-center gap-3">
                    ...
                    <span class="text-sm font-semibold text-gray-700">Actif dès la création</span>
                </label>
            </div>
