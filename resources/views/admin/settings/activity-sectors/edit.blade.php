...
@section('title', 'Modifier « ' . $activitySector->name . ' » - Administration')
...
<div class="min-h-screen bg-gray-50">
    <div class="mx-auto max-w-2xl px-6 py-8">
        ...
        <p class="text-sm font-medium text-[#284625]">
            ...
            /
            <a href="{{ route('admin.settings.activity-sectors.index') }}" class="hover:underline">Secteurs d’activité</a>
            /
            Modifier
        </p>
        <h1 class="mt-1 mb-6 text-3xl font-bold text-gray-900">Modifier « {{ $activitySector->name }} »</h1>

        <div class="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
            <form method="POST" action="{{ route('admin.settings.activity-sectors.update', $activitySector) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input type="text"
                           name="name"
                           value="{{ old('name', $activitySector->name) }}"
                           placeholder="Ex: Technologie / IT"
                ...
                <label class="flex items-center gap-3">
                    <input type="checkbox"
                           name="is_active"
                           value="1"
                           {{ old('is_active', $activitySector->is_active) ? 'checked' : '' }}
