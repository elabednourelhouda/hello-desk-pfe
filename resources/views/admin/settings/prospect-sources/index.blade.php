...
@section('title', 'Sources de prospects - Administration')
    ...
    <div class="mx-auto max-w-5xl px-6 py-8">
        ...
        <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-3xl font-bold text-gray-900">Sources de prospects</h1>
                <p class="mt-2 text-sm text-gray-500">
                    Ces valeurs alimentent le champ « Source du prospect » dans le formulaire
                    de création/édition d’un prospect.
                </p>
            </div>

            <a href="{{ route('admin.settings.prospect-sources.create') }}"
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($prospectSources as $prospectSource)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    {{ $prospectSource->prospects_count }}
                                </td>

                                <td class="px-6 py-4">
                                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold
                                        {{ $prospectSource->is_active
                                            ? 'bg-green-100 text-green-700 ring-1 ring-green-500/20'
                                            : 'bg-gray-100 text-gray-500 ring-1 ring-gray-400/20' }}">
                                        {{ $prospectSource->is_active ? 'Actif' : 'Inactif' }}
                                    </span>
                                </td>

                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.settings.prospect-sources.edit', $prospectSource) }}"
                                        ...
                                        <form method="POST"
                                              action="{{ route('admin.settings.prospect-sources.toggleActive', $prospectSource) }}">
                                            @csrf
                                            @method('PATCH')
                                            ...
                                            <button type="submit"
                                                ...
                                                {{ $prospectSource->is_active ? 'Désactiver' : 'Activer' }}
                                            </button>
                                        </form>

                                        <form method="POST"
                                              action="{{ route('admin.settings.prospect-sources.destroy', $prospectSource) }}"
                                              onsubmit="return confirm('Supprimer définitivement cette source ?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-14 text-center text-sm text-gray-500">
                                    Aucune source de prospect enregistrée pour le moment.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
