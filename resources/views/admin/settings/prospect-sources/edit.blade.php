...
@section('title', 'Modifier ' . $prospectSource->name . ' - Administration')
...
<div class="min-h-screen bg-gray-50">
    <div class="mx-auto max-w-2xl px-6 py-8">
        ...
        <p class="text-sm font-medium text-[#284625]">
            ...
            /
            <a href="{{ route('admin.settings.prospect-sources.index') }}" class="hover:underline">Sources de prospects</a>
        </p>
        <h1 class="mt-1 text-3xl font-bold text-gray-900">Modifier « {{ $prospectSource->name }} »</h1>

        <form method="POST"
              action="{{ route('admin.settings.prospect-sources.update', $prospectSource) }}"
            ...
            <div class="grid gap-5">
                <div>
                    ...
                    <input type="text"
                           name="name"
                           value="{{ old('name', $prospectSource->name) }}"
                           required
                           placeholder="Ex: Salon professionnel"
                ...
                <div>
                    ...
                    <input type="text"
                           name="code"
                           value="{{ old('code', $prospectSource->code) }}"
                           required
                           placeholder="Ex: salon_professionnel"
                           class="h-12 w-full rounded-xl border border-gray-300 px-4 text-sm shadow-sm focus:border-[#284625] focus:ring-[#284625]">
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                        ...
                        C’est cette valeur qui est réellement enregistrée sur chaque prospect.
                    </p>
                </div>
                ...
                <label class="flex items-center gap-3">
                    <input type="checkbox"
                           name="is_active"
                           value="1"
                           {{ old('is_active', $prospectSource->is_active) ? 'checked' : '' }}
                           class="h-5 w-5 rounded border-gray-300 text-[#284625] focus:ring-[#284625]">
                    <span class="text-sm font-semibold text-gray-700">Actif (visible dans le formulaire prospect)</span>
                </label>
            </div>
